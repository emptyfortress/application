...
.bl
	q-list()
		q-expansion-item(icon='mdi-script-text-outline' v-for='item in list' :key='item.id' popup)
			...
			q-card-actions
				q-btn(outline color='primary' label='Редактировать запрос' @click='request(item.label)' size='sm') 
				q-btn(outline color='primary' label='Редактировать представление' @click='view' size='sm') 
	br
